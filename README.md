...
## 特徴
- OpenGL ES による自前の地球儀レンダラ(NASA Blue Marble を 5 段階のタイルで同梱、オフラインで動作)
- ドラッグ回転・慣性・ピンチズーム・ダブルタップ拡大(Google Earth 風の「指の下の地点を保つ」操作)
- 6,000 語以上のピンを画面上で自動間引き・ラベル配置。ズームすると同じ場所の項目を円状に展開
- 用語検索(表記ゆれ・別名・地名・「ヴ/バ」表記に対応)、画面右下の縦バーによる年代の絞り込み(ハンドルのドラッグと数値入力に対応)、分類・地域による絞り込み、時代順/五十音順の一覧
- 詳細カードから Wikipedia(モバイル版)の記事と YouTube 検索をアプリ内 WebView で表示
- 用語の一覧(年代順/五十音順、分類・地域で絞り込み)、一度開いた用語への既読マーク、ブックマーク(最大 1,000 件)
- NASA Blue Marble を 5 段階のタイル(level 0〜4)で同梱し、より高倍率まで拡大可能
